...
//   // 十分之一机率炸弹
// }
function extandZero(block: BlockState) {
  if (BlockList.adjacentMines || block.revealed)
    return
...
function getSiblings(block: BlockState) {
  return directions.forEach(([dx, dy]) => {
    const x2 = block.x + dx
    const y2 = block.y + dy
...
function getBlockClass(item: BlockState) {
  ...
  return item.mine ? 'bg-red-300' : numberColor[item.adjacentMines]
}
